...
test('测试 parseOffsetAndLimit 方法', () => {
    ...
    // 非整数
    try {
        parser.parseOffsetAndLimit(1,1.3)
    }
    catch(err) {
        // console.log(err)
        expect(err.name).toBe('QueryPaserError')
        expect(err.fullMessage()).toMatch(/SizeDefError/)
        expect(err.fullMessage()).toMatch(/参数必须是一个整数/)
    }
})
...
test('测试 parseComparision 方法', () => {
    ...
    // key 错误
    try {
        parser.parseComparision('compare','id#',1)
    }
    catch (err) {
        // console.log(err)
    ...
    try {
        parser.parseComparision('compare','id#eq#',1)
    }
    catch (err) {
        expect(err.name).toBe('ComparisionParserError')
        expect(err.fullMessage()).toMatch(/OpDefError/)
    ...
    // 正确输出
    expect(parser.parseComparision('compare','id',1)).toEqual({
        left: 'id',
        operator: 'eq',
        right: 1
    })
    expect(parser.parseComparision('compare','views#gte',100)).toEqual({
        left: 'views',
        operator: 'gte',
        right: 100
    })
    expect(parser.parseComparision('query','content#like','%政治')).toEqual({
        left: 'content',
        operator: 'like',
        right: '%政治'
    })
    expect(parser.parseComparision('compare','id#in',[1,2,4,6,7,8])).toEqual({
        left: 'id',
        operator: 'in',
        right: [1,2,4,6,7,8]
    })
